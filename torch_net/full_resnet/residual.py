...
This version is portable and implements the specific 'WS' architecture from the paper.
It can dynamically use SyncBatchNorm for DDP or standard BatchNorm for single-process training.
"""
...
class ComplexResNet(nn.Module):
    """A DDP-compliant complex-valued ResNet model implementing the 'WS' architecture from the paper."""
    def __init__(self, block_class, activation_function, learn_imaginary_component, use_sync_bn=False, input_channels=3, num_classes=10):
        super(ComplexResNet, self).__init__()
        
        # --- Architecture Configuration (Paper Section 4.1, 'WS' model) ---
        self.initial_filters = 12  # 12 complex filters
        self.blocks_per_stage = [16, 16, 16] # 16 residual blocks per stage for 3 stages
        ...
        # Final number of channels after 2 downsampling events (12 * 2 * 2 = 48)
...
class RealResNet(nn.Module):
    """A DDP-compliant real-valued ResNet model implementing the 'WS' architecture from the paper."""
    def __init__(self, block_class, use_sync_bn=False, input_channels=3, num_classes=10):
        super(RealResNet, self).__init__()
        
        # --- Architecture Configuration (Paper Section 4.1, 'WS' model) ---
        self.initial_filters = 18 # 18 real filters
        self.blocks_per_stage = [14, 14, 14] # 14 residual blocks per stage for 3 stages
        ...
        # Final number of channels after 2 downsampling events (18 * 2 * 2 = 72)
        final_channels = self.initial_filters * (2**(len(self.blocks_per_stage) - 1))
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
